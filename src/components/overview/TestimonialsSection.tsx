...
export default function TestimonialsSection() {
  ...
  return (
    <section className="w-full bg-brand-bg py-24 sm:py-32 overflow-hidden">
            ...
            <div className="flex gap-4">
              <button 
                ...
                className={`w-14 h-14 rounded-2xl border flex items-center justify-center transition-all duration-300 group
                  ${isPrevDisabled 
                    ? 'border-brand-border bg-brand-card/50 opacity-50 cursor-not-allowed' 
                    : 'border-[rgba(182,49,6,0.1)] bg-brand-card hover:bg-brand/10 hover:border-[#B63106] cursor-pointer hover:scale-105 shadow-sm'
                  }`}
              >
                <ArrowLeft className={`w-6 h-6 transition-colors ${isPrevDisabled ? 'text-brand-text-muted' : 'text-brand-text group-hover:text-[#B63106]'}`} />
              </button>
              <button 
                onClick={handleNext}
              ...
              >
                <ArrowRight className={`w-6 h-6 transition-colors ${isNextDisabled ? 'text-brand-text-muted' : 'text-brand-text group-hover:text-[#B63106]'}`} />
              </button>
            </div>
          </div>
          ...
          <div className="relative w-full overflow-hidden mt-4">
            ...
            >
              {testimonials.map((testimonial) => (
                <motion.div
                  key={testimonial.id}
                  whileHover={{ y: -8, boxShadow: "0 20px 40px -15px rgba(182, 49, 6, 0.15)" }}
                  className="bg-brand-card border border-[rgba(182, 49, 6,0.06)] shadow-[0_10px_30px_-15px_rgba(182, 49, 6,0.08)] rounded-[2rem] p-10 flex flex-col gap-8 transition-all duration-300 h-full cursor-default shrink-0"
                ...
                >
                  ...
                  {/* Content */}
                  <p className="text-[1.15rem] text-brand-text-secondary leading-relaxed font-medium">
